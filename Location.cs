...
"Yên Bái",
        ...
        public void showAll()
        {
            cbbxLocation.DataSource = DataProvider.Ins.DB.DIADIEMs.Select(t =>
                t.TEN
            ).ToList();
        ...
        public bool CheckData()
        {
            ...
            if (txtbxName.Text.Trim().CompareTo(string.Empty) == 0 || cbboxProvince.SelectedIndex == -1 || img_data == null)
            {
                return false;
            }
            return true;
        ...
        private void cbbxLocation_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            if (index >= 0)
            {
                DIADIEM temp = DataProvider.Ins.DB.DIADIEMs.Where(x => x.TEN == cbbxLocation.Text).FirstOrDefault();
